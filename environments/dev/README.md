...
k3d를 사용한 개발 Kubernetes 환경 구축 및 관리 스크립트입니다.

## 🔑 개발 환경 계정 정보

| 서비스 | URL | Username | Password |
|--------|-----|----------|----------|
...
> ⚠️ 위 계정 정보는 개발 환경 전용입니다. 프로덕션 환경에서는 사용하지 마세요!
...
├── scripts/
│   ├── start-environment.sh    # 개발 환경 시작
│   ├── stop-environment.sh     # 개발 환경 중지
│   ├── cleanup.sh              # k3d 리소스 정리
│   ├── setup-sops-age.sh       # SOPS Age 키 설정 (개발 환경용)
...
### 2. SOPS 설정 (개발 환경용, 선택사항)

개발 환경에서 시크릿을 암호화하여 관리하려면:
...
개발 환경에서는 `values/postgresql.yaml`과 `values/redis.yaml`에 평문으로 시크릿을 관리할 수 있습니다.
...
개발 환경에서도 암호화하여 관리하려면 SOPS + Age를 사용하세요.
...
- `k8s-dev-k3d/values/*.yaml` - 개발 환경 최적화 values 파일
...
| 방법 | 장점 | 단점 | 사용 시나리오 |
|------|------|------|------------|
| **평문 관리** | 간단, 빠름 | Git에 커밋 불가 | 개발 환경만 |
| **SOPS + Age** | 암호화, Git에 커밋 가능 | 설정 필요 | 개발 + 팀 협업 |
